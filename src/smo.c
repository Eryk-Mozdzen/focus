...
void focus_smo_driver(struct focus_srv_position *srv, struct focus_event *event) {
    ...
    switch(event->type) {
        case FOCUS_EVENT_TYPE_FOC_START: {
            const float ls = 0.5f * (event->arg.foc_start.ld + event->arg.foc_start.lq);

            smo->a = focus_math_exp(-(event->arg.foc_start.rs / ls) * FOCUS_CONFIG_SAMPLING_PERIOD);
            smo->b = (1.f - smo->a) / event->arg.foc_start.rs;
            ...
            focus_biquad_design_lowpass(&smo->omega_e_filter, smo->params.bandwidth, FOCUS_CONFIG_SAMPLING_FREQUENCY);
            focus_biquad_start(&smo->omega_e_filter);
        } break;
        case FOCUS_EVENT_TYPE_FOC_LOOP: {
            const float dir_prev = focus_math_atan2(smo->e_ab_estimate[1], smo->e_ab_estimate[0]);

            const float *i_ab = event->arg.foc_loop.i_ab;
            const float *u_ab = event->arg.foc_loop.u_ab;
            ...
            smo->i_ab_estimate[0] = (smo->a * smo->i_ab_estimate[0]) + (smo->b * (u_ab[0] - smo->e_ab_estimate[0])) -
                                    (smo->params.observer.eta * focus_math_sign(i_ab_residual[0]));
            smo->i_ab_estimate[1] = (smo->a * smo->i_ab_estimate[1]) + (smo->b * (u_ab[1] - smo->e_ab_estimate[1])) -
                                    (smo->params.observer.eta * focus_math_sign(i_ab_residual[1]));

            smo->e_ab_estimate[0] =
                smo->e_ab_estimate[0] + ((smo->params.observer.g / smo->b) *
                                         (i_ab_residual[0] - (smo->a * smo->i_ab_residual_prev[0]) +
                                          (smo->params.observer.eta * focus_math_sign(smo->i_ab_residual_prev[0]))));
            smo->e_ab_estimate[1] =
                smo->e_ab_estimate[1] + ((smo->params.observer.g / smo->b) *
